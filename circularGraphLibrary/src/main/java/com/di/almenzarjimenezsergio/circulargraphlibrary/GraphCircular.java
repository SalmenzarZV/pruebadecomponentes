package com.di.almenzarjimenezsergio.circulargraphlibrary;
...
public class GraphCircular extends View {

    private final int DEFAULT_SIZE = 500;
    private int width, height;
    //private int graphColor = Color.RED;
    ...
    public GraphCircular(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray ta = this.getContext().obtainStyledAttributes(attrs, R.styleable.GraphCircular);
        this.percentage = ta.getInt(R.styleable.GraphCircular_percentage,0);
        ta.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(new RectF(rec.top, rec.left, rec.right, rec.bottom ), 0,
                anguloprev, true, p1);
        // Parte negativa del porcentaje (en azul)
        float angulopost = (360*(100-percentage))/(float)100.0;
        p1.setColor(Color.BLUE);
        //canvas.drawArc(new RectF(rec), anguloprev, angulopost, true, p2);
        canvas.drawArc(new RectF(rec.top, rec.left, rec.right, rec.bottom), anguloprev,
                angulopost, true, p1);
